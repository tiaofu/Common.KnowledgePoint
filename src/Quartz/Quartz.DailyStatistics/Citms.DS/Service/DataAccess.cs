...
namespace Citms.Data
{
	internal class DataAccess
	{
		...
		private OracleCommand CreateCommand(string formatSql, bool isStoreProcedure, Parameter[] paramers, OracleConnection connection)
		{
			...
			if(paramers != null && paramers.Length != 0)
			{
				...
				}).ToDictionary(p => isStoreProcedure ? p.Name : (":" + p.Name), p =>
				{
					...
					if(p.Value is Citms.Data.OracleType&&(Citms.Data.OracleType)p.Value==Citms.Data.OracleType.Cursor)
					{
						paramer = new OracleParameter(p.Name, ((Citms.Data.OracleType)p.Value).ToDbType());
		...
		{
			if(value == null||value is Citms.Data.OracleType&&(Citms.Data.OracleType)value==Citms.Data.OracleType.DbNull)
				return null;
